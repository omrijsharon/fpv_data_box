...
def button_command():
    ...
    #start progress bar
    popup_copy = tk.Toplevel()
    popup_copy.geometry('250x150')
    popup_copy.resizable(False, False)
    popup_copy.title("Copying files...")
    btn_cancel = tk.Button(popup_copy, text="Cancel", command=cancel_command)
    btn_cancel.place(x=98, y=110)

    lbl_fc_file_copy = tk.Label(popup_copy, text="FC: copying file: ")
    lbl_fc_file_copy.place(x=18, y=10)
    progress_var_fc = tk.DoubleVar()
    progress_var_fc.set(0)
    progress_bar_fc = ttk.Progressbar(popup_copy, variable=progress_var_fc, maximum=100)
    progress_bar_fc.place(x=20, y=30, width=210, height=20)

    lbl_goggles_file_copy = tk.Label(popup_copy, text="Goggles: copying file: ")
    lbl_goggles_file_copy.place(x=18, y=58)
    progress_goggles_var = tk.DoubleVar()
    progress_goggles_var.set(0)
    progress_bar_goggles = ttk.Progressbar(popup_copy, variable=progress_goggles_var, maximum=100)
    progress_bar_goggles.place(x=20, y=80, width=210, height=20)

    popup_copy.update()
    # popup_copy.pack_slaves()
